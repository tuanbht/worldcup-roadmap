...
8. `zoomToLod` (no `prev`) returns `overview` / `titles` / `detail` at representative zooms
   ...
   extremes.
9. **Hysteresis:** from `detail`, dipping just below the `detail` _enter_ threshold but above
   its _exit_ threshold (passing `prev:'detail'`) stays `detail`; only crossing below the exit
