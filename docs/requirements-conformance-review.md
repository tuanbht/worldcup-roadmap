...
- **H1 — Hard requirement "scroll-to-zoom" is broken.** `src/components/roadmap/RoadmapCanvas.tsx:91`
  ...
  mandate the *default* mode (wheel zooms, drag pans) and explicitly mark `panOnScroll` as the **optional Figma
...
- **M1 — Client fetch/polling hand-rolled, not TanStack Query.** `src/features/roadmap/hooks/useTournament.ts`
  ...
  *client* poll in `useQuery({ refetchInterval })`.
...
- **M3 — Bracket layout hand-rolled, not d3-hierarchy.** `src/features/roadmap/layout/bracket-layout.ts`.
  ...
  (Non-goal), but *positioning* is mapped. A mirrored-convergent bracket isn't `d3.tree`'s default shape, so a
...
- **L2** Semantic zoom / LOD (zoomable §5, *recommended*) not implemented — node detail is constant across zoom.
