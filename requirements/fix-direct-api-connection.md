...
the API is never reached. (`server/cors.ts` already exists, which is only needed for *direct cross-origin*
...
- `grep -rn "'/api/worldcup'\|\"/api/worldcup\|fetch(\`/api/" src` shows the only API URLs are built through
  `apiUrl()` — no bare relative API paths in the fetchers.
