...
- Desktop ≥1024 default (no-param) framing stays **byte-identical**; this only changes the _param-present_
  cold-load path.
...
`hasFramedRef` design rather than re-deriving it — the cold-load focus frame must be the _one_ automatic frame
