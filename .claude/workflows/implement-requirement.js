export const meta = {
  ...
  description: 'Run a requirement through the 7-stage gated TDD pipeline using the req-* agents (all opus, xhigh effort).',
  whenToUse: 'Fully-automated, background execution of plan → review → test → refactor → implement → review → final review.',
  phases: [
    ...
    { title: 'Implement', detail: 'implement to GREEN + gated code review (loop up to 3x)', model: 'opus' },
    { title: 'Final', detail: 'holistic review: tradeoffs, debt, risks, follow-ups', model: 'opus' },
  ],
}
...
for (let i = 1; i <= MAX_ITERS && !planApproved; i++) {
  ...
  await agent(`${ctx}\n\nWrite the implementation plan to ${runDir}/plan.md.${revisionNote}`,
    { ...TIER, agentType: 'req-planner', phase: 'Plan', label: `plan#${i}` })

  const review = await agent(
    `${ctx}\n\nReview ${runDir}/plan.md against the requirement. Write ${runDir}/plan-review.md. Return your verdict.`,
    { ...TIER, agentType: 'req-plan-reviewer', phase: 'Plan', label: `plan-review#${i}`, schema: REVIEW_SCHEMA })
...
if (!planApproved) return { stoppedAt: 'plan', planIters, reason: 'plan not approved within iteration cap', planFeedback }
...
for (let i = 1; i <= MAX_ITERS && !implApproved; i++) {
  ...
  const review = await agent(
    ...
    { ...TIER, agentType: 'req-impl-reviewer', phase: 'Implement', label: `impl-review#${i}`, schema: REVIEW_SCHEMA })

  if (review?.verdict === 'APPROVED') implApproved = true
  else implFeedback = (review?.requiredChanges || []).map((c, n) => `${n + 1}. ${c}`).join('\n')
}
log(`Implementation stage: ${implApproved ? 'APPROVED' : 'NOT APPROVED'} after ${implIters} iteration(s)`)
