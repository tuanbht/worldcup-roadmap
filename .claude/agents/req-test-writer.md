...
- **Do not write production/implementation code.** Stub only the minimal type/module surface needed for tests to _compile and run and then fail_ (e.g. an interface or an empty exported function that throws "not implemented"). Keep such stubs minimal and clearly marked.
...
- Confirmation the suite is RED, with a short snippet of the failure output proving it fails for the _right_ reason.
