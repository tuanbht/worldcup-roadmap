...
- Note the API `meta.provider` (`fifa` vs `mock`) — live FIFA data differs from the mock fixture and can explain label/value differences.
- The dev server caches the assembled tournament (idle TTL ~10 min); if data looks stale, note it rather than concluding a code bug.
